...
export function detectRequiredCapabilities(body) {
  ...
  // 0.5.125 (upstream 6498b312) — map a mime type to a required capability.
  ...
  const scanBlock = (b) => {
    ...
    if (t === "file" || t === "document" || t === "input_file") {
      // Infer modality from the embedded mime when available; fall back to pdf for generic files.
      let fmime = null;
