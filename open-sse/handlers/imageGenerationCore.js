...
export async function handleImageGenerationCore({
  ...
  // 0.5.107 (upstream 5306bd90) — executor-delegating adapters. Some providers
  // (Antigravity) need the executor's full request envelope (project id, session
  // id, requestType) + auth headers, which a plain buildUrl/buildHeaders adapter
  // can't reproduce. Those set `useExecutor: true` and we hand off to the proven
  // executor flow instead of building the request by hand.
  if (adapter.useExecutor && adapter.executeViaExecutor) {
    try {
      ...
      if (binaryOutput) {
        ...
        if (!b64 && first?.url) {
          try { b64 = await urlToBase64(first.url); } catch { /* fall through to JSON */ }
        }
        if (b64) {
