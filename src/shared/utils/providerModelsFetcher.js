 ...
 * @param {string} [provider] dashboard provider id — passed to the backend so
 *   the catalog fetch can follow the provider's egress proxy pool on hosts
 *   with fail-closed outbound firewalls. Without it the backend fetches direct.
 * @returns {Promise<Array<{ id: string, name: string, contextLength?: number }>>}
 */
export async function fetchSuggestedModels(fetcher, provider) {
  if (!fetcher?.url || !fetcher?.type) return [];

  const cacheKey = provider ? `${provider}:${fetcher.url}` : fetcher.url;
