...
import { Card, Button, Modal, Input, CardSkeleton, ModelSelectModal, Toggle, ConfirmModal, Select } from "@/shared/components";
...
// media. Default fallback adapted to our free vision-capable mmf/mimo-auto.
const CAPACITY_ADAPTER_CAPS = [
  ...
  // pdf/videoInput hidden — no translator support yet for those blocks.
];
const DEFAULT_FALLBACK_MODEL = "mmf/mimo-auto";
...
export default function CombosPage() {
  ...
  const fetchData = async () => {
    try {
      ...
      const normalized = { ...EMPTY_CAPACITY_ADAPTER };
  ...
  // Merge a per-combo strategy patch into settings.comboStrategies. When the
  // resulting strategy is back to default "fallback" with no extras, drop the
  // entry entirely so settings stay clean.
...
function ComboCard({ combo, activeProviders = [], copied, onCopy, onEdit, onDelete, strategy = {}, onSetStrategy }) {
...
function CapacityAdapterCap({ cap, entry, onChange, activeProviders }) {
  ...
  const handleRemove = (index) => {
    ...
    // Auto-refill with the default fallback so an enabled pool is never empty.
