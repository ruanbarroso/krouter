...
const OAUTH_TEST_CONFIG = {
  ...
  github: {
    ...
    authHeader: "Authorization",
    authPrefix: "Bearer ",
    extraHeaders: { "User-Agent": "kRouter", "Accept": "application/vnd.github+json" },
  },
  iflow: {
...
async function fetchWithConnectionProxy(url, options = {}, effectiveProxy = null) {
  // Nothing on this path had a deadline, so a provider whose endpoint blackholes
  // traffic left the Test button spinning forever and pinned a socket per retry.
  if (!options.signal) {
...
async function testApiKeyConnection(connection, effectiveProxy = null) {
  ...
  try {
    switch (connection.provider) {
      ...
      case "atomesus": {
        const res = await fetchWithConnectionProxy("https://api.atomesus.com/v1/models", { headers: { Authorization: `Bearer ${connection.apiKey}` } }, effectiveProxy);
        return { valid: res.ok, error: res.ok ? null : "Invalid API key" };
      }
